...
def simplify_sparse_matrix(rows,columns, values):
    seen_indices={} #dictionary to be filled at the first apparence of a row and colums pairing
...
class sparse_matrix:
    ...
    def __add__(self,other):
        return sparse_matrix(np.concatenate((self.rows,other.rows)),
    ...
    def line_sum(self): #same as row_sum
        return sm_sum(self.rows,self.columns,self.values,self.shape)
    ...
    def __mul__(self,other):
        ...
        prod = sparse_matrix(*multiply_sparse_matrices(self.rows,self.columns,self.values,other.rows,other.columns,other.values), self.shape)  
        prod.simplify()
        return prod
